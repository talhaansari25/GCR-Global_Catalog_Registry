  ...
  return (
    <div className='prodCard'>
      <img
        ...
        onClick={() => setSinglePgDisplay(!singlePgDisplay)}
      />
          ...
          <div className='spF1'>
            ...
            <div className="spInfo">
              <div className='spInfoHeader'>
                ...
                <span className='pcBadge'>{prods.category}</span>
              </div>

              <div className='spInfoDesc'>
                <p>
                  Lorem ipsum dolor sit amet, consectetur adipisicing elit. Provident
                  quaerat ducimus fuga beatae, explicabo quidem quae nam corrupti
                  repellat et.
                </p>
              </div>

              <div className="spInfoVals">
                <h1>₹{prods.dynamicFields.price}</h1>
                <p className='pcBadge2'>
                  &nbsp;
                  <i className="fa-regular fa-square-check"></i>
                  &nbsp;
                  <b>{prods.dynamicFields.availability}</b>
                  &nbsp;Units Left...&nbsp;
                </p>
                <p className='pcBadge2'>
              ...
              {prods.applicationMethod && (
                <div className="spInfoUsage">{prods.applicationMethod}</div>
              )}

              <div className="spInfoDetails">
                {prods.brand && (
                  <span className='spOtBadge'>{prods.brand}</span>
                )}
                {prods.warranty && (
                  <span className='spOtBadge'>{prods.warranty}</span>
                )}
                {prods.model && (
                  <span className='spOtBadge'>{prods.model}</span>
                )}
                {prods.color && (
                  <span className='spOtBadge'>{prods.color}</span>
                )}
              </div>
            ...
            <i
              onClick={() => setSinglePgDisplay(!singlePgDisplay)}
              className='fas fa-close spClose'
            ></i>
          </div>
        </div>
      )}
